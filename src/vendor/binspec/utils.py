# ===========================================================================
# VENDORED FROM binspec (github.com/kareemelbadry/binspec), the code behind
# El-Badry et al. 2018b. Copied verbatim; the ONLY edits are:
#   (1) data paths are resolved relative to THIS package directory (a module-
#       level _DATA constant) instead of binspec's relative './neural_nets' and
#       './other_data', so the package imports without an os.chdir into the
#       clone (REQUIREMENT: do not depend on /tmp/binspec).
# Everything else (the maths, the network forward passes, the continuum fit) is
# unchanged. Imports under Python 3.13.
# ===========================================================================
...
# Resolve the vendored data directories relative to this file. binspec used the
# process working directory; we make the package self-contained instead.
...
def read_in_neural_network(name='normalized_spectra'):
    ...
    if name == 'normalized_spectra':
        path = os.path.join(_HERE, 'neural_nets/NN_normalized_spectra.npz')
    elif name == 'unnormalized_spectra':
        path = os.path.join(_HERE, 'neural_nets/NN_unnormalized_spectra.npz')
    elif name == 'radius':
        path = os.path.join(_HERE, 'neural_nets/NN_radius.npz')
    elif name == 'Teff2_logg2':
        path = os.path.join(_HERE, 'neural_nets/NN_Teff2_logg2.npz')
    tmp = np.load(path)
...
def load_wavelength_array():
    ...
    '''
    path = os.path.join(_HERE, 'other_data/apogee_wavelength.npz')
    tmp = np.load(path)
...
def load_cannon_contpixels():
    ...
    '''
    path = os.path.join(_HERE, 'other_data/cannon_cont_pixels_apogee.npz')
    tmp = np.load(path)
...
def load_visit_wavelength():
    ...
    '''
    tmp = np.load(os.path.join(_HERE, 'other_data/apogee_visit_wavelength.npz'))
    wave = tmp['wave']
    tmp.close()
    return wave
