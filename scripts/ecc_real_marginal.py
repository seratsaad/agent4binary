...
Per-epoch component-label assignment is marginalized, not optimized, using the
tie inversion of the primary velocity as the alternative.
"""
...
deep = pd.read_csv(os.path.join(_ROOT, "resources/census/stage2_deep.csv"))
...
orb = pd.read_csv(os.path.join(_ROOT, "resources/census/orbit_deep_posteriors.csv"),
                  usecols=["sdss_id", "status", "P50", "K1", "n_ep"])
...
for idx, sid in enumerate(ids):
    ...
    if sid not in mj.index:
        continue
    t = parse(mj.loc[sid, "mjd_per_visit"])
...
print("wrote %s (%d systems)" % (a.out, len(rows)))
